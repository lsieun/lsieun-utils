package lsieun.utils.core.ds.tree;
...
public interface Treeable<T, S> {
    ...
    static <T, E extends Treeable<T, E>> void recursiveGetId(List<E> list, Set<T> set, Function<E, T> func) {
        if (isEmpty(list)) {
            return;
        }
    ...
    static <T, E extends Treeable<T, E>> void recursiveGetMap(List<E> list, Map<T, E> map, Function<E, T> func) {
        if (isEmpty(list)) {
            return;
        }
